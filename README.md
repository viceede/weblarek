...
-
<div align="left">
  <img src="./public/interfaces.svg" alt="Диаграмма интерфейсов" width="300" height="150">
</div>
...
<div align="left">
  <img src="./public/classes.svg" alt="Диаграмма классов" width="600" height="150">
</div>
...
Взаимодействие с данными происходит на уровне методов (т.е. непосредственный доступ к хранимым данным доступен внутри класса, а внешнее взаимодействие происходит через методы).
...
- `MainCatalog` - *Каталог товаров*
	...
	- методы:
	    ...
	    -   `saveSelectableProduct()` - сохранение товара для подробного отображения. Получает на вход сам товар `selectedProduct: IProduct`;
	    -   `getSelectableProduct()` - получение товара для подробного отображения. Возвращает товар для отображения либо `null` в случае отсутствия такого товара (`IProduct | null`);
 - `ProductsCart` - *Корзина*
	...
	-   методы:
	    ...
	    -   `saveProduct()` - добавление товара, который был получен в параметре, в массив корзины. Получает на вход сам товар `product: IProduct`.;
  ...
  - `Customer` - *Покупатель*
    ...
    - методы:
	    -   `saveData()` - сохранение данных в модели. Получает на вход опциональные данные для сохранения `Partial<ICustomer>`;
	    ...
	    -   `validateData()` - валидация данных. Получает на вход нужные данные валидации в виде объекта и возвращает объект ошибок для каждого поля.

Слой коммуникации
-
<div align="left">
  <img src="./public/communication.svg" alt="Диаграмма интерфейсов и класса на слое коммуникации" width="600" height="150">
</div>

- Для взаимодействия приложения с сервером используется класс `AppApi`, который использует композицию: внутри себя содержит объект, реализующий интерфейс `IApi` (экземпляр класса `Api`), через который выполняются HTTP-запросы.
